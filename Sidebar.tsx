...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggle, activeMode, setActiveMode, lang, isAdmin, activeModules, isMobileOpen, closeMobile }) => {
  ...
  const sidebarClasses = `
    fixed inset-y-0 left-0 z-[100] transform transition-transform duration-500 ease-out bg-[#020617] border-r border-white/5 flex flex-col overflow-hidden shadow-2xl
  ...
  const handleNav = (id: string, path: string) => {
    if (id in StudioMode) setActiveMode(id as StudioMode);
    navigate(path);
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[90] transition-opacity duration-300 lg:hidden ${isMobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={closeMobile}
      />
      ...
      <aside className={sidebarClasses}>
        <div className="p-8 flex items-center justify-between">
          ...
          {isMobileOpen && (
            <button onClick={closeMobile} className="lg:hidden p-2 text-slate-500 hover:text-white">
              <X className="w-6 h-6" />
            </button>
          )}
        </div>
          ...
          <div className="px-4 py-4">
            {userItems.map((item) => (
              ...
              >
                ...
                {(isOpen || isMobileOpen) && <span className="ml-5 font-black text-[10px] uppercase tracking-widest">{item.label}</span>}
            ...
            {growthItems.length > 0 && (
              <div className="mt-8 pt-8 border-t border-white/5">
                  <p className={`text-[8px] font-black uppercase tracking-[0.3em] text-slate-600 mb-4 px-4 ${!(isOpen || isMobileOpen) && 'text-center'}`}>
                    {(isOpen || isMobileOpen) ? 'Growth & Skills' : 'SKILLS'}
                  </p>
            ...
            {isAdmin && (
              <div className="mt-8 pt-8 border-t border-white/5">
                  <p className={`text-[8px] font-black uppercase tracking-[0.3em] text-slate-600 mb-4 px-4 ${!(isOpen || isMobileOpen) && 'text-center'}`}>
                    {(isOpen || isMobileOpen) ? 'Governance' : 'ROOT'}
                  </p>
                  <button
                  ...
                  >
                    ...
                    {(isOpen || isMobileOpen) && <span className="ml-5 font-black text-[10px] uppercase tracking-widest">{t.admin}</span>}
